...
assign is_last_w = (width_count==reg2dp_width);//number_of_total_trans_in_width-1);
//==============
...
wire [64:0] base_addr_w_w;
assign base_addr_w_w = base_addr_w + 64'd8;
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    base_addr_w <= {64{1'b0}};
    ...
    if (op_load) begin
        ...
        if (is_last_c) begin
            {mon_base_addr_w_c,base_addr_w} <= base_addr_w_w;
        end
    end
  end
end
...
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    ...
    if (op_load) begin
        ...
        if (is_last_c) begin
            {mon_dma_req_addr_c,dma_req_addr} <= base_addr_w_w;
        end else begin
...
//     end
// end
assign width_size = 3'd0; //req_size; // 1~8
assign dma_req_size = {{12{1'b0}}, 3'd0}; //req_size};
//==============
// Context Qeueu : Beats
//==============
