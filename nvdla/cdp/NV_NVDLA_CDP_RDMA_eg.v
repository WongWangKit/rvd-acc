...
//==============
assign lat_rd_prdy = lat_rd_pvld ;//& (~lat_rd_mask[0] | (lat_rd_mask[0] & ro0_wr_rdy)); 

assign ro0_wr_pvld = lat_rd_pvld & (lat_rd_mask[0] & ro0_wr_rdy);  
assign ro0_wr_rdy = 1'b1; //&ro0_wr_rdys;  
...
assign tran_rdy = (ro0_wr_pvld) || (is_last_tran & is_last_beat & dp_rdy);
...
assign is_cube_end = ig2eg_last_w & ig2eg_last_h & ig2eg_last_c;
...
assign dp_vld = ro0_wr_pvld;
assign dp_pd[64 -1:0] = lat_rd_data[64 -1:0]; //dp_data[64 -1:0];
assign dp_pd[64] = dp2reg_done_f;
// assign dp_pd[64 +3:64] = dp_pos_w[3:0];
// assign dp_pd[64 +7:64 +4] = dp_width[3:0];
// assign dp_pd[64 +12:64 +8] = dp_pos_c[4:0];
// assign dp_pd[64 +13] = dp_b_sync ;
assign dp_pd[65] = ig2eg_last_w & ig2eg_last_c;
// assign dp_pd[64 +15] = dp_last_h ;
assign dp_pd[66] = ig2eg_last_c ;
// assign dp_pd[64 +24:64 +17] = dp_invalid[7:0];
wire [68 -1:0] cdp_rdma2dp_pd_i;
assign cdp_rdma2dp_pd_i = {eg2ig_done_f, dp_pd};
//: my $k=1*8 +27;
...
reg [68-1:0] skid_flop_cdp_rdma2dp_pd_i;
reg pipe_skid_dp_vld;
reg [68-1:0] pipe_skid_cdp_rdma2dp_pd_i;
// Wire
wire skid_dp_vld;
wire [68-1:0] skid_cdp_rdma2dp_pd_i;
wire skid_dp_rdy_f;
wire pipe_skid_dp_rdy_f;
wire cdp_rdma2dp_valid_f;
wire [68-1:0] cdp_rdma2dp_pd_o;
// Code
// SKID READY
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
   if (!nvdla_core_rstn) begin
...
always @(posedge nvdla_core_clk) begin
    if (skid_flop_dp_rdy_f & dp_vld) begin
        skid_flop_cdp_rdma2dp_pd_i[68-1:0] <= cdp_rdma2dp_pd_i[68-1:0];
    end
end
assign skid_cdp_rdma2dp_pd_i[68-1:0] = (skid_flop_dp_rdy_f) ? cdp_rdma2dp_pd_i[68-1:0] : skid_flop_cdp_rdma2dp_pd_i[68-1:0];
...
always @(posedge nvdla_core_clk) begin
    if (skid_dp_rdy_f && skid_dp_vld) begin
        pipe_skid_cdp_rdma2dp_pd_i[68-1:0] <= skid_cdp_rdma2dp_pd_i[68-1:0];
    end
end
...
assign {eg2ig_done_flag, cdp_rdma2dp_pd} = cdp_rdma2dp_pd_o;
assign dp2reg_done_flag = cdp_rdma2dp_pd_o[64];
