...
input nvdla_core_rstn;
input cfg_prelu_en;
input [32:0] mul_data_in;
input [15:0] mul_op_in;
input bypass_trt;
...
wire [48:0] mul_prelu_out;
reg  p1_skid_catch_d1;
reg  bypass_trt_d1;
reg  p1_pipe_ready_bc_d1;
reg  p1_skid_pipe_valid_d1;
reg  bypass_trt_reg_d1;
reg  [48:0] mul_data_final_d1;
reg  p1_skid_ready_flop_d1;

always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    p1_skid_catch_d1      <= 'b0;
    bypass_trt_d1         <= 'b0;
    p1_pipe_ready_bc_d1   <= 'b0;
    p1_skid_pipe_valid_d1 <= 'b0;
    bypass_trt_reg_d1     <= 'b0;
    mul_data_final_d1     <= 'b0;
    p1_skid_ready_flop_d1 <= 'b0;
  end else begin 
    p1_skid_catch_d1      <= p1_skid_catch     ;
    bypass_trt_d1         <= bypass_trt        ;
    p1_pipe_ready_bc_d1   <= p1_pipe_ready_bc  ;
    p1_skid_pipe_valid_d1 <= p1_skid_pipe_valid;
    bypass_trt_reg_d1     <= bypass_trt_reg    ;
    mul_data_final_d1     <= mul_data_final    ;
    p1_skid_ready_flop_d1 <= p1_skid_ready_flop;
  end
end
...
// VCS sop_coverage_off start
  p1_skid_data <= (p1_skid_catch_d1)? {mul_prelu_out[48:0],bypass_trt_d1} : p1_skid_data;
...
always @(*) begin
  ...
  {mul_data_final[48:0],bypass_trt_reg} = (p1_pipe_ready_bc_d1 && p1_skid_pipe_valid_d1) ? 
                                              (p1_skid_ready_flop_d1) ? {mul_prelu_out[48:0], bypass_trt_reg_d1} : p1_skid_data :
                                          {mul_data_final_d1[48:0],bypass_trt_reg_d1};
end
